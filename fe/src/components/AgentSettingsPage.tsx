...
export function AgentSettingsPage({
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-col bg-[var(--agenthub-bg)]" data-testid="agent-settings-page">
      <header className="flex min-h-16 items-center justify-between border-b border-[var(--agenthub-outline)] bg-[var(--agenthub-surface-0)] px-4">
        <div>
          <h2 className="text-lg font-semibold text-[var(--agenthub-foreground)]">Agent Profile</h2>
          <p className="text-xs text-[var(--agenthub-muted)]">维护运行配置、模型和环境变量</p>
        </div>
        <button
          data-testid="back-to-chat-button"
          type="button"
          onClick={onBackToChat}
          className="inline-flex h-9 cursor-pointer items-center justify-center rounded-md border border-[var(--agenthub-outline)] bg-[var(--agenthub-surface-0)] px-3 text-sm font-medium text-[var(--agenthub-foreground)] transition hover:bg-[var(--agenthub-surface-2)]"
        >
          返回聊天
        </button>
      </header>
        ...
        <div className="mx-auto grid max-w-7xl gap-5 xl:grid-cols-[280px_minmax(0,1fr)_360px]">
          <section className="min-w-0">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-base font-semibold text-[var(--agenthub-foreground)]">Profile</h3>
              <span className="text-xs text-[var(--agenthub-muted)]">{agentProfiles.length} 个</span>
            </div>
            <div className="overflow-hidden rounded-md border border-[var(--agenthub-outline)] bg-[var(--agenthub-surface-0)] shadow-[var(--agenthub-elevation-1)]" data-testid="agent-profile-list">
              {agentProfiles.map((profile) => (
                <button
                  ...
                  className={`block w-full cursor-pointer border-b border-slate-100 px-3 py-3 text-left last:border-b-0 ${
                    selectedProfile?.id === profile.id ? 'bg-[var(--agenthub-primary-container)]' : 'bg-[var(--agenthub-surface-0)] hover:bg-[var(--agenthub-surface-2)]'
                  }`}
                >
                  <span className="block truncate text-sm font-medium text-[var(--agenthub-foreground)]">{profile.label}</span>
                  <span className="mt-1 block truncate font-mono text-xs text-[var(--agenthub-muted)]">{profile.id}</span>
                  <span className="mt-2 inline-flex items-center rounded-sm bg-[var(--agenthub-surface-2)] px-1.5 py-0.5 text-[11px] font-medium text-[var(--agenthub-muted)]">
                    {profile.type}
                  </span>
                </button>
              ))}
              {!agentProfiles.length ? <div className="px-3 py-8 text-center text-sm text-[var(--agenthub-muted)]">暂无 Profile</div> : null}
            </div>
          ...
          <AgentSettingsDetail
            ...
            onAddProfileModel={onAddProfileModel}
            onDeleteProfileModel={onDeleteProfileModel}
...
interface AgentSettingsDetailProps {
  /** connectionState 表示当前 WebSocket 连接状态。 */
  connectionState: ConnectionState
  /** backendVersion 表示后端构建版本。 */
  backendVersion: string
...
// AgentSettingsDetail 使用 props 参数渲染选中 Profile 的编辑区。
function AgentSettingsDetail({
  ...
  onDeleteProfileModel,
  onSetProfileDefaultModel,
}: AgentSettingsDetailProps) {
  const [labelDraft, setLabelDraft] = useState(selectedProfile?.label ?? '')
  const [typeDraft, setTypeDraft] = useState<AgentProfileType>(selectedProfile?.type ?? 'claude_code')
  const [commandDraft, setCommandDraft] = useState(selectedProfile?.command ?? '')
  const [argsDraft, setArgsDraft] = useState((selectedProfile?.args ?? []).join('\n'))
  const [envDraft, setEnvDraft] = useState(envToText(selectedProfile?.env ?? []))
