...
func runTokenAuthCase(ctx E2EContext) (success bool) {
	...
	steps := make([]string, 0)
	defer func() {
		writeTokenAuthReport(ctx.OutputDir, success, steps)
	}()
	...
	steps = append(steps, "AGENTHUB_TOKEN 非空时，前端先展示 token 输入界面，不连接工作台。")
	...
	steps = append(steps, "输入错误 token 时，前端保持在 token 输入界面，且不会持久化错误 token。")
	...
	steps = append(steps, "输入正确 token 后，前端连接 WebSocket 并只把 AGENTHUB_TOKEN 写入持久化状态。")
	...
	steps = append(steps, "刷新页面后，前端复用已储存的 AGENTHUB_TOKEN，不再储存其它状态。")
	return true
}

// writeTokenAuthReport 使用 outputDir、success 和 steps 参数写入 Token 鉴权测试报告。
func writeTokenAuthReport(outputDir string, success bool, steps []string) {
	report := []string{
		"# Token 鉴权 E2E 测试报告",
		"",
