...
func runSettingsBackendVersionCase(ctx E2EContext) (success bool) {
	...
	events = append(events, reportStep("进入设置页后，可以看到后端返回的版本信息。"))
	events = append(events, reportImage("后端版本信息", "screenshots/01-backend-version.png"))
	return true
}
...
func expectBackendVersionVisible(page playwright.Page, timeout time.Duration) error {
	...
	for time.Now().Before(deadline) {
		text, err := getByTestID(page, "backend-version-text").TextContent()
		if err == nil {
	...
	if lastErr != nil {
		return fmt.Errorf("等待后端版本信息超时，最后错误: %w", lastErr)
	}
	return fmt.Errorf("等待后端版本信息超时，实际文本: %s", lastText)
}
